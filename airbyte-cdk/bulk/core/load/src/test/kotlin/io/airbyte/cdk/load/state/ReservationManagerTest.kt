...
class ReservationManagerTest {
    ...
    fun testReserveBlocking() = runTest {
        val memoryManager = ReservationManager(1000)
        val reserved = AtomicBoolean(false)

        try {
            withTimeout(5000) { memoryManager.reserveFor(900, this) }
        } catch (e: Exception) {
        ...
        val job = launch {
            memoryManager.reserveFor(200, this)
            reserved.set(true)
        }

        memoryManager.reserveFor(0, this)
    ...
    fun testReserveBlockingMultithreaded() = runTest {
        ...
        withContext(Dispatchers.IO) {
            memoryManager.reserveFor(1000, this)
            Assertions.assertEquals(0, memoryManager.remainingMemoryBytes)
            val nIterations = 100000

            val jobs =
                (0 until nIterations).map { launch { memoryManager.reserveFor(10, this) } }
    ...
    fun testRequestingMoreThanAvailableThrows() = runTest {
        val memoryManager = ReservationManager(1000)
        try {
            memoryManager.reserveFor(1001, this)
    ...
    fun testReservations() = runTest {
        val memoryManager = ReservationManager(1000)
        val reservation = memoryManager.reserveFor(100, this)
