...
class MsSqlServerFieldTypeMapper : JdbcMetadataQuerier.FieldTypeMapper {
    override fun toFieldType(c: JdbcMetadataQuerier.ColumnMetadata): FieldType {
        when (val type = c.type) {
            is SystemType -> {
                ...
                log.info{"type ${c.type} is a systemType. returning $retVal"}
                return retVal
            }
            else -> {
                log.info{"type ${c.type} is a not systemType"}
    ...
    enum class MsSqlServerSqlType(val names: List<String>, val jdbcType: JdbcFieldType<*>) {
        ...
        constructor(jdbcType: JdbcFieldType<*>, vararg names: String) : this(names.toList(), jdbcType) {
        }

        companion object {
            private val nameToValue = MsSqlServerSqlType.entries.flatMap {
                msSqlServerSqlType -> msSqlServerSqlType.names.map {
                    name -> name to msSqlServerSqlType
                }
            }.toMap()

            fun fromName(name: String?): MsSqlServerSqlType? {
                val retVal = nameToValue[name]
                log.info{"type $name maps to $retVal"}
                return retVal
            }
        }
    }
