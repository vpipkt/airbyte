...
@JsonSubTypes(
    JsonSubTypes.Type(value = EncryptionDisabled::class, name = "unencrypted"),
    JsonSubTypes.Type(
        value = EncryptionRequiredTrustServerCertificate::class,
...
data object EncryptionDisabled : MsSqlServerEncryptionConfigurationSpecification

@JsonSchemaTitle("Encrypted (trust server certificate)")
@JsonSchemaDescription(
    "Use the certificate provided by the server without verification. (For testing purposes only!)"
)
data object EncryptionRequiredTrustServerCertificate :
...
class MicronautPropertiesFriendlyMsSqlServerEncryption {
    ...
    fun asEncryption(): MsSqlServerEncryptionConfigurationSpecification =
        when (mode) {
            "preferred" -> EncryptionDisabled
            "required" -> EncryptionRequiredTrustServerCertificate
